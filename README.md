...
These elements are described in different folders. All the modelling happens within the `openfisca_pakistan` folder.
...
```sh
pip install openfisca_pakistan
```
...
```sh
git clone https://github.com/openfisca/openfisca-pakistan.git
cd openfisca-pakistan
...
Substitute your package's country name for `openfisca_pakistan` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_pakistan/situation_examples/couple.json \
